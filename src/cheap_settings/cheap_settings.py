...
def _convert_value_to_type(value, to_type):
    ...
    if value is not None and isinstance(value, str):
        if issubclass(to_type, int):
            return int(value)
        elif issubclass(to_type, float):
            return float(value)
        ...
        # TODO: Deal with Optional types & type unions (& algebraic types) by using `issubclass`
            ...
            if issubclass(to_type, list) and not isinstance(parsed_value, list):
                raise ValueError(f"{value} is not a list")
            if issubclass(to_type, dict) and not isinstance(parsed_value, dict):
