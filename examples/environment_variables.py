...
def main():
    ...
    print(f"Database Host: {Settings.database_host}")
    print(f"Database Port: {Settings.database_port} (type: {type(Settings.database_port).__name__})")
    print(f"Connection Timeout: {Settings.connection_timeout}s (type: {type(Settings.connection_timeout).__name__})")
    ...
    for key in ["DATABASE_HOST", "DATABASE_PORT", "CONNECTION_TIMEOUT", "USE_SSL", "MAX_CONNECTIONS"]:
